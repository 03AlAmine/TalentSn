...
import { AuthService, CvData } from '../../core/services/auth.service';
import { CvUploadComponent } from '../candidats/cv/cv-upload/cv-upload.component';
import { ExtractedProfile } from '../../core/services/cv-parser.service';
...
export class OnboardingComponent {
  ...
  async onProfileExtracted(profile: ExtractedProfile): Promise<void> {
    ...
    if (profile.experiences.length > 0 && profile.experiences[0].title)
      this.experiences = profile.experiences.map((e) => ({
        title: e.title || '',
        company: e.company || '',
    ...
    if (profile.educations.length > 0 && profile.educations[0].degree)
      this.educations = profile.educations.map((e) => ({
        degree: e.degree || '',
    ...
    const hasContent =
      (profile.experiences.length > 0 && profile.experiences[0].title) ||
  ...
  removeCertification(i: number) {
    if (this.certifications.length > 1) this.certifications.splice(i, 1);
  }
  addTechnicalSkill(s: string) {
    if (s.trim() && !this.technicalSkills.includes(s.trim()))
      this.technicalSkills.push(s.trim());
  }
  removeTechnicalSkill(i: number) {
    this.technicalSkills.splice(i, 1);
  }
  addSoftSkill(s: string) {
    if (s.trim() && !this.softSkills.includes(s.trim()))
      this.softSkills.push(s.trim());
  }
  ...
  removeLanguage(i: number) {
    if (this.languages.length > 1) this.languages.splice(i, 1);
  }
